...
Binnen de component [TaskForm.vue](https://github.com/yan-nick-91/vue_todo_with_mock_db/blob/main/src/views/components/task/form/TaskForm.vue) heb deze logic toegepast, waarbij er een check wordt gedaan of er bullet points aan de lijst is toegevoegd. Deze kijkt of de lengte meer dan 0 is, voor dat de `<div>` element wordt gerenderd. `<div v-if="bulletList.length > 0">`.
...
Met dit voorbeeld — rechtstreeks afkomstig van [TaskForm.vue](https://github.com/yan-nick-91/vue_todo_with_mock_db/blob/main/src/views/components/task/form/TaskForm.vue) — is te zien dat bij het class attribuut een v-bind:class is wordtgebruikt en hierdoor nu dynamisch is.
